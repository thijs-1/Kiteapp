"""Configuration constants for data pipelines."""
from pathlib import Path
import numpy as np

# Project paths
PROJECT_ROOT = Path(__file__).parent.parent
DATA_DIR = PROJECT_ROOT / "data"
RAW_DATA_DIR = DATA_DIR / "raw"
PROCESSED_DATA_DIR = DATA_DIR / "processed"
HISTOGRAMS_1D_FILE = PROCESSED_DATA_DIR / "histograms_1d.pkl"  # Single 3D array
HISTOGRAMS_2D_DIR = PROCESSED_DATA_DIR / "histograms_2d"
HISTOGRAMS_TEMPERATURE_FILE = PROCESSED_DATA_DIR / "histograms_temperature.pkl"
HISTOGRAMS_PRECIPITATION_FILE = PROCESSED_DATA_DIR / "histograms_precipitation.pkl"

# Day of year mapping: list of "MM-DD" strings for all 366 days
DAYS_OF_YEAR = [f"{m:02d}-{d:02d}" for m in range(1, 13) for d in range(1, 32)
                if not (m == 2 and d > 29) and not (m in [4, 6, 9, 11] and d > 30)]

# Input data
INPUT_SPOTS_FILE = PROJECT_ROOT / "windguru_spots.pkl"
ENRICHED_SPOTS_FILE = PROCESSED_DATA_DIR / "spots.pkl"

# Airport reference data (pre-filtered: large/medium airports with scheduled
# service and IATA codes; see scripts/build_airports_csv.py) and route cache.
# Source: OurAirports CSV, filtered at ingest to keep the file small
# (~3.3K rows instead of 85K rows / 12 MB).
AIRPORTS_REFERENCE_FILE = DATA_DIR / "airports.csv"
AIRPORT_ROUTES_CACHE_FILE = PROCESSED_DATA_DIR / "airport_routes_cache.json"
# OSRM /table computes one spot -> N airports in a single request, so the full
# enrichment run needs ~1 request per spot instead of 1 per (spot, airport) pair.
OSRM_TABLE_URL = "https://router.project-osrm.org/table/v1/driving"
NEAREST_AIRPORTS_COUNT = 3
HAVERSINE_SHORTLIST = 8
OSRM_RATE_LIMIT_SECONDS = 1.0
OSRM_REQUEST_TIMEOUT = 15
OSRM_MAX_ATTEMPTS = 3  # Total attempts per OSRM call (1 initial + retries on transient failures)
OSRM_RETRY_BACKOFF_SECONDS = 1.0  # Initial backoff; doubles each retry

# Wind strength bins: [0, 2.5, 5, 7.5, ..., 35, inf]
WIND_BINS = list(np.arange(0, 37.5, 2.5)) + [float('inf')]

# Direction bins: [-5, 5, 15, 25, ..., 345, 355] (36 bins of 10 degrees)
DIRECTION_BINS = list(range(-5, 360, 10))

# Daytime temperature bins: [-20, -17.5, ..., 45, inf] (2.5 degree Celsius increments)
TEMPERATURE_BINS = list(np.arange(-20, 47.5, 2.5)) + [float('inf')]

# Hourly precipitation bins: a dry bin [0, 0.01), then trace rain up to 0.5,
# then 0.5 mm increments to 5, then everything heavier: [5, inf)
PRECIPITATION_BINS = [0.0, 0.01] + list(np.arange(0.5, 5.5, 0.5)) + [float('inf')]

# Grid configuration (larger cells to reduce CDS queue requests)
GRID_SIZE_LAT = 90  # degrees (2 cells latitude)
GRID_SIZE_LON = 60  # degrees (6 cells longitude)
GRID_EXPANSION_DEGREES = 0.5  # degrees to expand each grid cell for downloads

# Unit conversion
MS_TO_KNOTS = 1.94384
KELVIN_TO_CELSIUS_OFFSET = 273.15
M_TO_MM = 1000.0

# ERA5 configuration
ERA5_YEARS = 10  # Last 10 years of data
ERA5_DATASET = "reanalysis-era5-single-levels"
ERA5_VARIABLES = [
    "10m_u_component_of_wind",
    "10m_v_component_of_wind",
    "2m_temperature",
    "total_precipitation",
]

# ARCO download chunk configuration
ARCO_CHUNK_MONTHS = 3  # Download in 3-month chunks (~9 GB compressed per chunk)

# Checkpoint file for resumable ARCO pipeline
CHECKPOINT_FILE = DATA_DIR / "arco_checkpoint.json"

# Intermediate data (per-spot time series extracted from ERA5)
TIMESERIES_DIR = DATA_DIR / "timeseries"

# Daylight filtering configuration
# When enabled, only wind data during daylight hours is included in histograms
FILTER_DAYLIGHT_HOURS = True

# Sun depression angle for twilight definition (degrees below horizon)
# 0 = geometric sunrise/sunset (sun center at horizon)
# 6 = civil twilight (recommended for kitesurfing - enough light for activities)
# 12 = nautical twilight
# 18 = astronomical twilight
DAYLIGHT_DEPRESSION_ANGLE = 6.0
